defmodule Req.DecodeTest do
  ...
  describe "json" do
    ...
    test "invalid" do
      ...
      assert err == %JSON.DecodeError{message: "invalid byte 98"}
      assert resp.status == 200
      assert resp.body == ""
    end
  end
  ...
  defmodule LegacyDecoder do
    def decode(body), do: {:ok, String.upcase(body)}
  end

  test "custom decoder (legacy module exporting decode/1)" do
    %{req: req} =
      serve(
        "GET /": fn conn ->
          conn
    ...
    {resp, io} =
      ExUnit.CaptureIO.with_io(:stderr, fn ->
        Req.stream!(req, decoders: [ics: LegacyDecoder])
      end)
  ...
  defmodule UpcaseDecoder do
    ...
    def decode_init(:stream), do: :stream

    def decode_chunk({:buffer, buffer}, data), do: {:ok, data, {:buffer, [buffer | data]}}

    def decode_chunk(:stream, data), do: {:ok, String.upcase(data), :stream}

    def decode_finish({:buffer, buffer}) do
      {:ok, buffer |> IO.iodata_to_binary() |> String.upcase()}
    end

    def decode_finish(:stream), do: {:ok, nil}
