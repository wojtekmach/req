...
defmodule Req.Mint do
  ...
  def run(request) do
    ...
    case legacy_connect(request) do
      {:ok, conn} ->
  ...
  defp legacy_connect(request) do
  ...
  defp send_request(request, conn) do
    ...
    case Mint.HTTP.request(conn, method, request_path(request.url), headers, body) do
      {:ok, conn, ref} ->
        case legacy_stream_request_body(request, conn, ref, stream_body) do
          {:ok, request, conn} ->
  ...
  defp legacy_stream_request_body(request, conn, _ref, nil) do
    {:ok, request, conn}
  end

  defp legacy_stream_request_body(request, conn, ref, req_body_fun)
       when is_function(req_body_fun, 1) do
    case req_body_fun.(request) do
      {:data, chunk, request} ->
        case Mint.HTTP.stream_request_body(conn, ref, chunk) do
          {:ok, conn} ->
            legacy_stream_request_body(request, conn, ref, req_body_fun)
  ...
  defp legacy_stream_request_body(request, conn, ref, enumerable) do
    result =
      Enum.reduce_while(enumerable, {:ok, conn}, fn chunk, {:ok, conn} ->
        case Mint.HTTP.stream_request_body(conn, ref, chunk) do
          {:ok, conn} ->
  ...
  defp recv_into_nil(request, conn, ref, timeouts) do
    ...
    case legacy_recv_stream(conn, ref, {request, acc}, fun, timeouts) do
      {:ok, {request, acc}} ->
        response =
          Req.Response.new(
            status: acc.status,
  ...
  defp recv_into_fun(request, conn, ref, timeouts, fun) do
    ...
    case legacy_recv_stream(conn, ref, {request, Req.Response.new()}, stream_fun, timeouts) do
      {:ok, {request, response}} ->
        {request, response}
  ...
  defp recv_into_collectable(request, conn, ref, timeouts, collectable) do
    ...
    case legacy_recv_stream(conn, ref, {request, {nil, Req.Response.new()}}, stream_fun, timeouts) do
  ...
  # Streams response entries into fun until :done, :halt, or error. Headers
  # received after the first body chunk are passed as {:trailers, fields}.
  defp legacy_recv_stream(conn, ref, acc, fun, timeouts) do
    case legacy_recv_loop(conn, ref, :headers, acc, fun, timeouts) do
      {:ok, conn, acc} ->
        Mint.HTTP.close(conn)
        {:ok, acc}
  ...
  defp legacy_recv_loop(conn, ref, phase, acc, fun, timeouts) do
    case Mint.HTTP.recv(conn, 0, recv_timeout(timeouts)) do
      {:ok, conn, entries} ->
        case handle_entries(entries, ref, phase, acc, fun) do
          {:cont, phase, acc} ->
            legacy_recv_loop(conn, ref, phase, acc, fun, timeouts)
  ...
  defp legacy_apply_fun(event, rest, ref, phase, acc, fun) do
    case fun.(event, acc) do
      {:cont, acc} ->
        handle_entries(rest, ref, phase, acc, fun)
