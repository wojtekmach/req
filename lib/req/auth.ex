defmodule Req.Auth do
  ...
  def legacy_auth(request) do
    legacy_auth(request, request.options[:auth])
  end

  defp legacy_auth(request, nil) do
    request
  end

  defp legacy_auth(request, authorization) when is_binary(authorization) do
    Req.Request.put_header(request, "authorization", authorization)
  end

  defp legacy_auth(request, {:basic, userinfo}) when is_binary(userinfo) do
    Req.Request.put_header(request, "authorization", "Basic " <> Base.encode64(userinfo))
  end

  defp legacy_auth(request, {:bearer, token}) when is_binary(token) do
    Req.Request.put_header(request, "authorization", "Bearer " <> token)
  end

  defp legacy_auth(request, {:digest, userinfo}) when is_binary(userinfo) do
    request
  end

  defp legacy_auth(request, fun) when is_function(fun, 0) do
    value = fun.()
    ...
    legacy_auth(request, value)
  end

  defp legacy_auth(request, {mod, fun, args})
  ...
  defp legacy_auth(request, :netrc) do
    path = System.get_env("NETRC") || Path.join(System.user_home!(), ".netrc")
    authenticate_with_netrc(request, path)
  end

  defp legacy_auth(request, {:netrc, path}) do
    authenticate_with_netrc(request, path)
  end

  defp legacy_auth(request, {username, password})
