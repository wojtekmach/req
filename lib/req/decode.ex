defmodule Req.Decode do
  ...
  defp format(request, response) do
    ...
    case Req.Response.get_header(response, "content-type") do
      [content_type | _] ->
        case legacy_extensions(content_type, path) do
          [ext | _] -> ext
          [] -> nil
        end

      [] ->
        case legacy_extensions("application/octet-stream", path) do
          [ext | _] -> ext
          [] -> nil
        end
    end
  ...
  defp legacy_extensions("application/octet-stream" <> _, path) do
    if tgz?(path) do
      ["tgz"]
    else
      path |> MIME.from_path() |> MIME.extensions()
  ...
  defp legacy_extensions("application/" <> subtype, path) when subtype in ~w(gzip x-gzip) do
    if tgz?(path) do
      ["tgz"]
    else
      ["gz"]
  ...
  defp legacy_extensions(content_type, _path) do
    MIME.extensions(content_type)
  end
end
