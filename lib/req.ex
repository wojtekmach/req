defmodule Req do
  ...
  Response body options:
    ...
    * `:decoders` - the list of decoders to use for automatic response body decoding.
      Defaults to `[:json, :json_api]`. See `Req.Decode` for the supported
  ...
  def stream(req, acc, fun, options \\ []) when is_function(fun, 3) do
    ...
    if req.into != nil do
      raise ArgumentError, "Req.stream/4 does not support :into option"
    end
    ...
    case run_stream(req, acc, stream_fun) do
      ...
      {:error, err, resp, acc} ->
        case run_error_steps(err, resp) do
          {:ok, resp} ->
            {:ok, resp, acc}
  ...
  def stream(req, options \\ []) do
    if options[:into] == :self do
      raise ArgumentError, "Req.stream/2 does not support into: :self"
    end
    ...
    case run_stream(req, %Req.Buffer{}, stream_fun) do
      {:ok, resp, %Req.Buffer{} = buffer} ->
        resp = put_in(resp.body, Req.Buffer.body(buffer))
        run_response_steps(resp)

      {:error, err, resp, %Req.Buffer{} = buffer} ->
        resp = put_in(resp.body, Req.Buffer.body(buffer))
        run_error_steps(err, resp)
    end
  end
  ...
  defp do_stream(%{adapter: adapter} = req, [], acc, fun, state) when is_function(adapter, 4) do
    adapter.(req, acc, fun, state)
  end
  ...
  defp run_response_steps(%Req.Response{} = resp) do
    case run_response(resp.request, resp) do
  ...
  defp run_error_steps(exception, %Req.Response{} = resp) do
    case run_error(resp.request, exception) do
      {req, %{__exception__: true} = exception} ->
        resp = put_in(resp.request, req)
        {:error, exception, resp}
  ...
  defp run_response(request, response) do
    Enum.reduce_while(request.response_steps, {request, response}, fn {_name, step},
                                                                      {request, response} ->
  ...
  defp run_error(request, exception) do
    Enum.reduce_while(request.error_steps, {request, exception}, fn {_name, step},
                                                                    {request, exception} ->
